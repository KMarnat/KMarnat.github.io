    ...
    .then((weatherData) => {
      if (!weatherData.name) {
        ...
        if (currentSky.textContent) {
          ...
          will be a sun*/
          if (
            (weatherData.weather[0].main === "Clear" && searchDate.getHours() >= searchedSunset.getHours()) ||
            (weatherData.weather[0].main === "Clear" && searchDate.getHours() <= searchedSunrise.getHours())
          ) {
      ...
      lon.textContent = `Longitude: ${weatherData.coord.lon}`;
      lat.textContent = `Latitude: ${weatherData.coord.lat}`;
